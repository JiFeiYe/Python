...
# X,y分别存放特征向量和标签. 注:这⾥使用了reshape(-1,1), 其主要的原因是
# data[:,0]是⼀维的数组(因为只有⼀个特征),但后⾯调用模型的时候对特征向量的要求
# 是矩阵的形式,所以这⾥做了reshape的操作。
X, y = data[:, 0].reshape(-1, 1), data[:, 1]
# 使用train_test_split函数把数据随机分为训练数据和测试数据
# 训练数据的占⽐由参数train_size来决定。如果这个值等于0.8,就意味着随机提取80%的数据作为训练集
X_train, X_test, y_train, y_test = train_test_split(X, y, train_size=0.8)
# TODO 1. 请实例化⼀个线性回归的模型
regr = linear_model.LinearRegression()
# TODO 2. 在X_train,y_train上训练⼀个线性回归模型。 如果训练顺利,则regr会存储训练完成之后的结果模型
regr.fit(X_train, y_train)
# 在训练集上做验证,并观察是否训练得当,⾸先输出训练集上的决定系数R平⽅值
